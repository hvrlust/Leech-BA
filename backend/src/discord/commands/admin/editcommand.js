...
module.exports = {
    name: 'editcommand',
    description: 'edits an existing custom copypasta command',
    help: '```asciidoc\nIn no particular order \n' +
        '-name :: the name of the command \n' +
        '-response [optional] :: bot\'s response when calling the command \n' +
        ...
        '\n\nExample usages: \n'+ADMIN_PREFIX+'editcommand -name time -deleteafter -response the time is currently -description ' +
    ...
    execute: async (bot, message, params) => {
        ...
        if(name.length !== 1){
            await message.reply("that command requires -name argument followed by the name, use `"+
                ADMIN_PREFIX+"help editcommand` for examples");
            return;
        }

        if(!commandAlreadyExists(name[0], bot.commands)) {
            await message.reply("command doesn't exist!");
            return;
        }
