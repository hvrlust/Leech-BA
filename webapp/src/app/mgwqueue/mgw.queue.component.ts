...
@Component({
  ...
  templateUrl: './mgw.queue.component.html',
  styleUrls: ['./mgw.queue.component.scss'],
  providers: [ CookieService ]
})
export class MgwQueueComponent {
  lastRefresh = 'Loading';
  lastRefreshBy = {};
  ...
  columnDefs = [
    ...
    {headerName: 'Services required', field: 'need', autoHeight: true, cellRendererFramework: MgwServiceComponent,
    ...
    {headerName: 'Action', field: 'action', width: 86, cellRendererFramework: MgwActionComponent,
      editable: false,
  ...
  dataList = {};
  queueUpdater;
  constructor(public dialog: MatDialog, private queueService: MgwQueueService, private cookieService: CookieService) {
  ...
  openDelete(id, rsn): void {
    const dialogRef = this.dialog.open(MgwConfirmationComponent, {
      width: '400px',
      disableClose: true,
    });
  ...
  openDemote(id, rsn): void {
    const dialogRef = this.dialog.open(MgwDemoteConfirmComponent, {
      width: '400px',
      disableClose: true,
    });
    dialogRef.componentInstance.rsn = rsn;
  ...
  openAdd(): void {
    const dialogRef = this.dialog.open(MgwQueueAddComponent, {
      width: '550px',
      disableClose: true,
    });
    dialogRef.afterClosed().subscribe(result => {
  ...
  openEdit(data): void {
    const dialogRef = this.dialog.open(MgwQueueEditComponent, {
      width: '550px',
      disableClose: true,
      data: data,
    });
...
@Component({
  selector: 'mgw-app-service-element',
...
export class MgwServiceComponent implements ICellRendererAngularComp {
  public params: any;
...
@Component({
  selector: 'mgw-app-action-element',
  ...
  <i class="fas fa-arrow-down btn padding-right v-center" (click)="demote(params.data.id, params.value.rsn)"></i>`
})
export class MgwActionComponent implements ICellRendererAngularComp {
  public params: any;
  refresh(params: any): boolean {
    return false;
  }
  ...
  public demote(id, rsn) {
    this.params.context.componentParent.openDemote(id, rsn);
  }
  public edit(rsn) {
